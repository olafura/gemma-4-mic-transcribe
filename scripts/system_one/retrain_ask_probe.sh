...
#    the 3,089 base rows are reused from data/system-one/cache-router-probe.
#    The held-out cache is built once, on first use (400 rows, ~5 min).
# 3. Export OUT_DIR/ask-probe from base + new rows.
# 4. Score the shipped probe and the new one side by side on the held-out
...
eval_caches=("$heldout_cache")
eval_labels=($D/router-probe-heldout.jsonl)
if [ ${#eval_files[@]} -gt 0 ]; then
...
"${PY[@]}" scripts/system_one/export_ask_probe.py \
  --cache "$base_cache" "$out/cache-train-extra" \
  --labels $D/router-probe-train.jsonl "$out/train-extra.jsonl" \
