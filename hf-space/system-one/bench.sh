#!/usr/bin/env bash
# Times the packed-int4 kernels alone (q4_bench.exs, or BENCH_SCRIPT such as
# head_bench.exs) on an Nvidia GPU, with the same CUDA setup as job.sh, whose
# setup half it runs.
#
...
/app/system_one/bin/system_one eval "Code.eval_file(\"/data/jobs/${BENCH_SCRIPT:-q4_bench.exs}\")" 2>&1 |
