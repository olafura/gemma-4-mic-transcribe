...
def main():
    ...
    for m in manifest:
        ...
        if windows:
            ...
            for k in range(n):
                ...
                if float(np.sqrt(np.mean(chunk ** 2))) < 1e-3:  # skip silence
                    continue
                res = post(url, wav_bytes(chunk), candidates); used += 1
                for r in res["languages"]:
                    logp[r["language"]] += math.log(max(r["probability"], 1e-9))
            if used == 0:
                res = post(url, wav_bytes(samples), candidates)
        ...
        row = {"file": m["file"], "language": lang, "label": m["label"], "seconds": m["seconds"],
               ...
               "probabilities": probs, "ms": ms, "known": True}
        rows.append(row)
    ...
    print(f"all: {n} clips, top-1 {100*t1/n:.1f}%, top-3 {100*t3/n:.1f}%, p50 {lat[n//2]} ms, p95 {lat[int(n*0.95)]} ms, wall {time.time()-t_all:.0f} s")
    summary = {"clips": n, "top1": t1 / n, "top3": t3 / n, "p50_ms": lat[n // 2], "p95_ms": lat[int(n * 0.95)]}
